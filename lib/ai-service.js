...
CRITICAL: Return ONLY valid JSON. No markdown, no explanations, just the JSON object.`;
  ...
  /**
   * Generate structured interview questions with natural flow (PHASE 2 ENHANCED)
   * Flow: Greeting → Resume Discussion → Projects → Behavioral → Technical → Closing
...
TASK: Generate ${numQuestions} interview questions following a NATURAL INTERVIEW FLOW:

**INTERVIEW STRUCTURE:**
1. **Greeting Phase** (1 question): Warm introduction, make candidate comfortable
   - "Tell me about yourself and walk me through your background"
   
2. **Resume Discussion Phase** (${Math.max(1, Math.floor(numQuestions * 0.3))} questions): About their resume, overview of experience
   - Reference their resume: "I see you have experience at ${experience[0]?.company || 'previous companies'}"
   - Ask about career journey, transitions
   
3. **Projects Deep-Dive Phase** (${Math.max(1, Math.floor(numQuestions * 0.3))} questions): DETAILED questions about specific projects
   - MUST reference SPECIFIC project names: "${projects[0]?.projectName || 'your recent project'}"
   - Ask about technologies used, architecture, challenges, solutions
   - Example: "Tell me about your '${projects[0]?.projectName}' project. What was your role, what tech stack did you use, and what were the biggest challenges?"
   
4. **Behavioral Phase** (${Math.max(1, Math.floor(numQuestions * 0.2))} questions): Soft skills, teamwork, problem-solving
   - Based on their actual work experience
   - Leadership, conflict resolution, learning
   
5. **Technical Phase** (${Math.max(1, Math.floor(numQuestions * 0.2))} questions): Technical depth on their skills
   - About specific technologies from their resume: "${techSkills.split(',')[0]}"
   - System design, best practices, trade-offs
...
- Enable follow-up conversations (design questions that invite elaboration)
...
[
  {
    ...
    "phase": "greeting|resume-discussion|projects|behavioral|technical|closing",
    "category": "projects|experience|skills|general",
    "context": "Brief note about what this relates to"
  }
]

IMPORTANT: Return ONLY the JSON array. No markdown, no explanations. Questions MUST follow the phase order above.`;

    const systemMessage = `You are an expert technical interviewer who conducts natural, conversational interviews. You structure interviews in a logical flow: starting with warm introductions, then discussing the resume, deep-diving into projects, exploring behavioral aspects, testing technical knowledge. You always reference specific details from the candidate's background to make it personal.`;
